...
public class Point {
    ...
    // O(nbOfPoints)
    // randomly generating point across the canvas
    public static ArrayList<Point> generateArrayOfRandomPoints(int nbOfPoints, int width, int height, int widthOfLine,
            Random random) {
        if (nbOfPoints > 0) {
            ArrayList<Point> tmp = new ArrayList<>();
            for (int i = 0; i < nbOfPoints; i++) {
                tmp.add(generateRandomPoint(Math.max(0, width - widthOfLine), Math.max(0, height - widthOfLine),
                        random));
            }
            return tmp;
        }
    ...
    // O(nbOfPoints)
    // creating points with the same x and y coordinates
    // point generation function that pairs well with a squared shape canvas
    public static ArrayList<Point> generateDiagonalOfPoints(int nbOfPoints, int width, int height, int widthOfLine,
            Random random) {
        if (nbOfPoints > 0) {
            ArrayList<Point> tmp = new ArrayList<>();
            for (int i = 0; i < nbOfPoints; i++) {
                int randomCoord = random.nextInt((Math.min(width, height) - 0) + 1) + 0;
                tmp.add(new Point(Math.max(0, randomCoord - widthOfLine),
                        Math.min(Math.max(randomCoord - widthOfLine, 0), width - widthOfLine)));
            }
            return tmp;
        }
        return null;
    ...
    // O(nbOfPoints)
    // 4 if conditions as we want to change x and y values of new points by adding or subtracting sometimes
    public static ArrayList<Point> generateCluster(int nbOfPoints, int width, int height, int widthOfLine,
            Random random, double clusterProbability) {
        if (nbOfPoints > 0) {
            ArrayList<Point> tmp = new ArrayList<>();
