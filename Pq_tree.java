...
public class Pq_tree {
    ...
    Pq_tree(ArrayList<Point> pointsInZone, int widthOfImage, int heightOfImage, int nbOfPoints, int widthOfLine,
        ...
        this.root = new Pq_node(pointsInZone, widthOfImage, heightOfImage, 0, 0, widthOfImage, heightOfImage);

        this.generateSubtrees(this.root);

    }

    static public Pq_tree generateBetterRandomTree(ArrayList<Point> pointsInZone, int widthOfImage, int heightOfImage, int nbOfPoints, int widthOfLine, double sameColorProbability,
    long seed){
        return new Pq_tree(pointsInZone, widthOfImage, heightOfImage, nbOfPoints, widthOfLine, sameColorProbability, seed);
    }
        ...
        if (node != null) {
            if (node.pointsInZone.size() >= 0) {
                ...
                node.addTopLeftChild(topLeftPoints, node.center.getX() - node.startX, node.center.getY() - node.startY,
                        node.startX, node.startY, node.center.getX(), node.center.getY());
                ...
                node.addBottomRightChild(bottomRightPoints, node.endX - (node.center.getX() + this.widthOfLine),
                        ...
                        node.center.getY() + this.widthOfLine, node.endX, node.endY);
